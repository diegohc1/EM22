...
tdesc <- within(tdesc, {prop = round(prop, 1)}) #redondeamos
...
matriz_cfa <- matriz[which(matriz$Analisis2 %in% c("CFA", "PCA")), ] #solo CFA o PCA
matriz_cfa <- matriz_cfa[complete.cases(matriz_cfa[, "Cod_indice"]) ,] #retiramos si no tienen cod_indice
matriz_cfa <- matriz_cfa[c("Concatena1", "Constructo_indicador", "Enunciado", "sub_escala", "cod_gen", "cod_preg", "Cod_indice", "Cod_indice2", "Analisis2")]  
names(matriz_cfa)[which(names(matriz_cfa) == "Constructo_indicador")] <- "Constructo"

#mismas bases en miau y lista
matriz_cfa <- matriz_cfa[which(matriz_cfa$Concatena1 %in% names(lista)), ] 
lista <- lista[unique(matriz_cfa$Concatena1)]
matriz_lista <- split(matriz_cfa, matriz_cfa$Concatena1) #separamos matriz en listas
matriz_lista <- lapply(matriz_lista, as.data.frame)
...
#para guardar 
ins <- lapply(lista, function(x) NULL) #para guardar los insumos del RT
puntajes <- lapply(lista, function(x) NULL) 
puntajes_lista <- NULL
prec <- lapply(lista, function(x) NULL)
...

{
  ...
  for(i in 1:length(nom)){ #i=3
    ...
    matriz_i <- matriz_lista[[i]]
    
    vcod_indice <- unique(matriz_i$Cod_indice) #escalas del cuestionario i
    tipo <- matriz_i[row.names(unique(matriz_i["Cod_indice"])), "Analisis2"] #para identifcar pca o cfa despues
    ...
    for(j in 1:length(vcod_indice)){ #j=2
      ...
      acomoda_string_lavaan(preg)
      
      mod2 <- split(matriz_i, matriz_i$Cod_indice) %>% map(acomoda_string_lavaan)
      
      
      
      ee <- lapply(mod2, function(x) cfa(x, bd, ordered = TRUE, mimic = "Mplus", estimator = "WLSMV"))
      lapply(ee, reporte_lavaan)
      ...
      clusterExport(cl, list('mod2', 'bd'))
      bb1 <- parLapply(cl, mod2, function(x) cfa(x, bd, ordered = TRUE, mimic = "Mplus", estimator = "WLSMV"))
      bb1 <- lapply(bb1, lavPredict)
      stopCluster(cl)
      })
      
      system.time({
      bb2 <- lapply(mod2, function(x) cfa(x, bd, ordered = TRUE, mimic = "Mplus", estimator = "WLSMV"))
      bb3 <- lapply(bb2, lavPredict)
            })
